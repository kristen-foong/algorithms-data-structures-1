...
void sortString(string stuff, bool check[], int end){
   if(end < 0){
      ...
      if(isdigit(stuff[end])){
         ...
         if(isalpha(stuff[end])){
            int ascii = (((int) stuff[end]) % 65) + 10;
            //cout << "test2: " << ascii << endl;
            if(check[ascii] == false){
               check[ascii] = true;
            }
         }
      }
...
int main(){
   ...
   string stuff2 = upperCases(stuff, end);
   sortString(stuff2, check, end);
}