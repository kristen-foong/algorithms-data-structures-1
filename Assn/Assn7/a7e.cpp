...
int main(){
   ...
   cout << "check isEmpty: " << isEmpty(s) << endl; // there is no war in ba sing se
   add(5,s);//add
   add(7,s);//add
   add(3,s);//add
   add(9,s);//add
   ...
   add(12,s); //looks into camera like im on the office
   printList(s);
   cout << endl;
   ...
   cout << "check ithNode: "; //checks ithNode
   node*n = makeDummy();
   push(0,n);
   push(1,n);
   push(2,n);
   ...
   cout << "check deleteList: "; //checks deleteList
   deleteList(n); // i tried
   printList(n);
   cout << endl;
